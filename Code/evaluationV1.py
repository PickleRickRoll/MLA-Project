...
input_shape = (
    ...
    60,
    1,
)  # Example input shape (time frames, frequency bins, channels)
...
predicted_Yo, predicted_Yn, predicted_Yp = predictions


# Évaluation
def evaluate_onsets(true_Yo, predicted_Yo, threshold=0.5):
    predicted_onsets = (predicted_Yo > threshold).astype(int)
    precision = precision_score(true_Yo.flatten(), predicted_onsets.flatten())
    recall = recall_score(true_Yo.flatten(), predicted_onsets.flatten())
    f1 = f1_score(true_Yo.flatten(), predicted_onsets.flatten())
    return precision, recall, f1


precision, recall, f1 = evaluate_onsets(Y_test_Yo, predicted_Yo)
print(f"Onset Evaluation - Precision: {precision}, Recall: {recall}, F1: {f1}")


# Visualisation
def plot_activation_matrix(matrix, title):
    plt.figure(figsize=(10, 5))
    plt.imshow(matrix.squeeze().T, aspect="auto", origin="lower", cmap="hot")
    plt.colorbar()
    plt.title(title)
    plt.xlabel("Temps")
...
plot_activation_matrix(predicted_Yo[0], "Onsets prédits")
plot_activation_matrix(predicted_Yn[0], "Notes prédites")
plot_activation_matrix(predicted_Yp[0], "Multipitch prédit")
