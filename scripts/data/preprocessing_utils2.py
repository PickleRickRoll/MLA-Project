...
def process_audio(audio_file, sr=22050, hop_size=512, segment_length=2.0):
    """
    ...
    cqt = librosa.cqt(y, sr=sr, hop_length=hop_size, n_bins=84, bins_per_octave=12)
    cqt = np.abs(cqt).T  # Transpose to get (time frames x frequency bins)
    
    # Split into segments
    num_frames = cqt.shape[0]
    segment_frames = int(segment_length * sr / hop_size)
    segments = [cqt[i:i + segment_frames] for i in range(0, num_frames, segment_frames) if i + segment_frames <= num_frames]
...
def create_dataset(audio_dir, midi_dir, sr=22050, hop_size=512, segment_length=2.0):
    """
    ...
    for audio_file, midi_file in zip(audio_files, midi_files):
        ...
        # Process audio
        audio_segments = process_audio(audio_path, sr, hop_size, segment_length)
        time_resolution = hop_size / sr
        num_frames = int(segment_length * sr / hop_size)
        
        # Process MIDI
        onset_posteriorgram, activation_posteriorgram, pitch_posteriorgram = process_midi(midi_path, num_frames, time_resolution)
    ...
    y_train = {
        "onset_output": np.array(y_onsets),
        "activation_output": np.array(y_activations),
        "pitch_output": np.array(y_pitches),
    }
